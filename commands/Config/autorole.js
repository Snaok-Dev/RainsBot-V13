...
module.exports.run = async (client, message, args, data) => {
    if (!data.plugins.autorole.enabled) return message.channel.send(`⚠️ Le plugin d'autorôle n'est pas activé. Utilisez \`${data.prefix}enable autorole\` pour l'activer !`);
    ...
    if (args.length) {
        ...
        if (newRole.id === data.plugins.autorole.role) return message.channel.send('⚠️ Ce rôle est déjà défini comme autorôle.');
        ...
        message.channel.send(`✅ L'autorôle a été configuré avec succès. Le rôle \`@${newRole.name}\` sera attribué aux nouveaux membres !`);
    } else {
...
module.exports.help = {
    ...
    aliases: ["auto-role"],
    category: 'Config',
    description: "Ajoute un autorôle sur le serveur",
    usage: "<role>",
    cooldown: 5,
