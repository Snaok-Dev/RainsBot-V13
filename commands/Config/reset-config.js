...
module.exports.run = async (client, message) => {
    ...
    const c = new MessageCollector(message.channel, { filter, time: 30000, max: 1 });
    ...
    c.on("collect", async msg => {
        if (msg.content.toLowerCase() === "oui") {
            ...
            message.channel.send('✅ La configuration a bien été réinitialisée.');
        } else {
            c.stop(true);
...
module.exports.help = {
    ...
    category: 'Config',
    description: "Remettre la configuration du serveur à zéro",
    usage: "",
    cooldown: 5,
