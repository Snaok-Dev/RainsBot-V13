module.exports.run = async (client, message, args, data) => {
    let plugin = args[0];
    if (!plugin) return message.channel.send('⚠️ Ce plugin n\'existe pas. Voici la liste des plugins: `welcome`, `goodbye`, `logs`, `autorole`, `suggestion`. \nVous ne trouvez pas ce que vous voulez ? Faites `' + data.prefix + 'config` pour voir les autres configurations. Le module de protection s\'active séparément via les commandes `raidmode`, `antigiverole`, `antiban`, `antilink`.');
    ...
    switch (plugin.toLowerCase()) {
        ...
        case "logs": {
            if (data.plugins.logs.enabled) {
                ...
                if (!message.guild.me.permissions.has("EMBED_LINKS") || !message.guild.me.permissions.has("VIEW_AUDIT_LOG")) return message.channel.send('J\'ai besoin des permissions Intégrer des liens et Voir les logs du serveur pour pouvoir activer les logs!');
        ...
        case "autorole": {
            if (data.plugins.autorole.enabled) {
                ...
                if (!message.guild.me.permissions.has("MANAGE_ROLES")) return message.channel.send('J\'ai besoin des permissions Gérer les rôles pour pouvoir activer l\'autorole!');
